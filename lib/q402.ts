// lib/q402.ts
import { verifyTypedData, createWalletClient, http, parseEther, defineChain } from 'viem';
...
export async function settlePayment(payload: Q402Payload) {
  try {
    ...
    const chain = payload.paymentDetails.networkId === 'monad-mainnet' ? monadMainnet : monadTestnet;
    ...
    if (amountBig > parseEther("0.1")) { // Max 0.1 MON Settlement
    ...
    // For this Demo on Monad Testnet, the Facilitator
